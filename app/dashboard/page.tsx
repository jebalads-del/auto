...
export default function DashboardPage() {
  ...
  const handleCreateAd = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!newTitle || !newPrice) return alert('الرجاء إدخال عنوان الإعلان والسعر')
    setFormLoading(true)
    ...
    try {
      const res = await fetch('/api/ads', {
        ...
        body: JSON.stringify({ 
          ...
          description: newDesc,
          image_url: newImage || 'https://unsplash.com'
        })
      })
      const result = await res.json()
      if (result.success) {
        alert('تم نشر إعلان السيارة بنجاح في قاعدة البيانات!')
        setNewTitle('')
        setNewPrice('')
        setNewDesc('')
        setNewImage('')
      ...
      <div style={{ backgroundColor: 'white', padding: '15px', borderRadius: '10px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
        ...
        {activeTab === 'create_ad' && (
          <form onSubmit={handleCreateAd} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '14px' }}>عنوان السيارة (مثال: تويوتا كامري 2024):</label>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '14px' }}>رابط صورة السيارة عبر الإنترنت (اختياري):</label>
              <input type="text" value={newImage} onChange={(e) => setNewImage(e.target.value)} placeholder="https://example.com" style={{ width: '100%', padding: '10px', border: '1px solid #cbd5e1', borderRadius: '4px', boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '14px' }}>مواصفات وتفاصيل السيارة المتاحة:</label>
            ...
            <button type="submit" disabled={formLoading} style={{ padding: '12px', backgroundColor: '#10b981', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold', fontSize: '15px', opacity: formLoading ? 0.7 : 1 }}>
              {formLoading ? 'جاري الحفظ في السحابة...' : 'نشر الإعلان فوراً وبشكل حي 🚀'}
            </button>
          </form>
        )}
        ...
        {activeTab === 'payment' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <h3 style={{ color: '#334155', margin: 0 }}>💳 إعداد بوابات تحويل واستقبال الأموال</h3>
            <div style={{ border: '1px solid #cbd5e1', padding: '15px', borderRadius: '8px', backgroundColor: '#f8fafc' }}>
              <h4 style={{ margin: '0 0 10px 0', color: '#003087' }}>حساب PayPal</h4>
            ...
            <div style={{ border: '1px solid #cbd5e1', padding: '15px', borderRadius: '8px', backgroundColor: '#f8fafc' }}>
              <h4 style={{ margin: '0 0 10px 0', color: '#ffb300' }}>تحويل Western Union</h4>
            ...
            <button onClick={() => alert('تم التحديث!')} style={{ padding: '12px', backgroundColor: '#f59e0b', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold', width: '100%' }}>
              حفظ وتأكيد بيانات الدفع
            </button>
          </div>
        )}
